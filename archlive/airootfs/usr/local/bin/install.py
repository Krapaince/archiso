...
def partition_and_format_disk() -> Tuple[Device, bool]:
    ...
    return (disk, crypt_root_partition)
...
def copy_conf_script():
    shutil.copy(
        CONFIGURE_SCRIPT_FILEPATH,
        join_absolute(ROOT_MOUNTPOINT, SCRIPT_PATH),
    )
    shutil.copy(UTILS_SCRIPT_FILEPATH, join_absolute(ROOT_MOUNTPOINT, SCRIPT_PATH))
    join_absolute(ROOT_MOUNTPOINT, CONFIG_PATH).mkdir(parents=True, exist_ok=True)
    shutil.copy(CONFIG_FILEPATH, join_absolute(ROOT_MOUNTPOINT, CONFIG_PATH))

    pacman_conf_filepath = Path("/etc/pacman.conf")
    shutil.copy(
        pacman_conf_filepath, join_absolute(ROOT_MOUNTPOINT, pacman_conf_filepath)
    )
...
def remove_conf_script():
    os.remove(join_absolute(ROOT_MOUNTPOINT, CONFIGURE_SCRIPT_FILEPATH))
    os.remove(join_absolute(ROOT_MOUNTPOINT, UTILS_SCRIPT_FILEPATH))
    os.remove(join_absolute(ROOT_MOUNTPOINT, CONFIG_FILEPATH))
...
def main():
    ...
    update_system_clock()
    (device, crypt_root_partition) = partition_and_format_disk()
    generate_fstab()
    install_essential_package()
    copy_conf_script()
    chroot(device, crypt_root_partition)
    remove_conf_script()
